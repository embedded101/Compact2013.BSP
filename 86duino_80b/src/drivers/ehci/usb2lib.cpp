...
PVOID USB2lib::AddedTt( UCHAR uHubAddress,UCHAR uPort)
{
    PVOID pContext = NULL;;
    Lock();
    
    while ((pContext = (PVOID)InterlockedIncrement(&m_lRef)) == NULL);
    ASSERT(pContext!=NULL);
    
    if (GetTT( uHubAddress,uPort,pContext) == NULL) {
        TransactionTrasnlate * pNewTT = new TransactionTrasnlate(uHubAddress,uPort,pContext, pTTRoot);
        if ( pNewTT) {
            pTTRoot=pNewTT;
        }
        else
...
BOOL USB2lib::DeleteTt( UCHAR uHubAddress,UCHAR uPort,PVOID pContext)
{
    BOOL bReturn = FALSE;
    Lock();
    ...
    TransactionTrasnlate * pCurTT = pTTRoot;
    while ( pCurTT!=NULL ) {
        if (pCurTT->GetHubAddress()==uHubAddress && pCurTT->GetHubPort() == uPort && pCurTT->GetContext()==pContext )
            break;
        else {
            pPrevTT = pCurTT;
...
TransactionTrasnlate * USB2lib::GetTT( const UCHAR uHubAddress,const UCHAR uHubPort, const PVOID pContext )
{
    Lock();
    TransactionTrasnlate * pFoundTT = pTTRoot;
    while (pFoundTT) {
        if (pFoundTT->GetHubAddress() == uHubAddress && pFoundTT->GetHubPort() == uHubPort && pFoundTT->GetContext() ==pContext )
            break;
        else
...
int USB2lib::AllocUsb2BusTime( IN const UCHAR uHubAddress,IN const UCHAR uHubPort,const PVOID ttContext, LPEndpointBuget ep)
{
    int retv;
    ...
    if (ep->speed == HSSPEED) {
        ...
        ep->calc_bus_time = (USHORT)Compute_nonsplit_overhead(ep) + (USHORT)Add_bitstuff(ep->max_packet);
...
void USB2lib::FreeUsb2BusTime(IN const UCHAR uHubAddress,IN const UCHAR uHubPort,const PVOID ttContext, LPEndpointBuget ep)
{
    Lock();
